...
class CLstmEncoder(GeneralModel):
    ...
    def __init__(self, dummy_input, 
        ...
        self.sequantial_block.to(dummy_input.device)
        ...
        # store the cout so that the we can have access to the data
        self.cout = C_out
...
class LstmEncoderDecoder(GeneralModel):
    ...
    def forward(self, X):
        N, T, C = X.shape
        outputs = torch.zeros(N, self.out_len, self.out_size)

        hidden, cell = self.encoder(X)

        # start token
        inputs = X[:, -1, :].unsequeeze(1)
